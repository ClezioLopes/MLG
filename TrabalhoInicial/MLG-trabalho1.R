####################################################################
data$Outro_2 = rnorm(364,50,3)
write.csv2(data,row.names = F,
           file="C:\\Users\\Clézio Lopes\\Desktop\\Clézio Lopes\\Curso Estatística\\8º Semestre\\MLG\\dadosmilho1.csv")
...
Estimar_f = function(converge) {
  ...
  #Estimando Beta1
  B0 = round(solve( t(m_x) %*% w0 %*% m_x) %*% (t(m_x) %*% w0 %*% z0),6)
  ...
  #Estimando Beta2
  B1 = round(solve( t(m_x) %*% w1 %*% m_x) %*% (t(m_x) %*% w1 %*% z1),6)
  ...
  while (abs(parada) > converge) {
    ...
    #Estimando Beta
    Beta = round(solve( t(m_x) %*% w %*% m_x) %*% (t(m_x) %*% w %*% z),6)
